 ...
 *   1. Directional (Δ) — delta × (currentProb - fillProb) × qty
 *   2. Time Decay (Θ)  — theta × daysSinceFill × qty
 *   3. Residual        — totalPnl - directional - timeDecay
 *
...
interface Attribution {
  directional: number   // Δ × (currentProb - fillProb) × qty × sign
  timeDecay: number     // Θ × daysSinceFill × qty × sign
...
function computeAttribution(
  ...
  for (const pos of positions) {
    ...
    // Directional: delta (per-unit pp sensitivity) × prob move × qty
...
function PositionBar({ position, maxAbs }: { position: Position; maxAbs: number }) {
  ...
  const shortTitle = position.marketTitle.length > 36
    ? position.marketTitle.slice(0, 34) + '…'
